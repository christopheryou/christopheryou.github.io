...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  for (let i = 0; i < testimonialsItem.length; i++) {
    ...
    testimonialsItem[i].addEventListener("click", function () {
      ...
      const itemDate = this.querySelector("[data-testimonials-date]");
      const modalCite = this.querySelector("[data-testimonials-cite]")

      if (itemDate) {
        modalDate.innerHTML = itemDate.innerHTML;
        modalDate.setAttribute("datetime", itemDate.getAttribute("datetime"));
      }
      else {
        modalDate.innerHTML = null;
      }

      if (modalCite) {
        modalText.innerHTML = modalCite.innerHTML;
      }
      else {
  ...
  for (let i = 0; i < projectsItem.length; i++) {
    projectsItem[i].addEventListener("click", function () {
      // Reset everything
      projectModalVid.src = "";
      projectModalVid.style.display = "none";   // hide video by default
      projectModalImg.src = "";
      projectModalImg.style.display = "none";   // hide image by default
      ...
      const videoEl = this.querySelector("[data-projects-video]");
      if (videoEl?.src) {
        projectModalVid.src = videoEl.src;
        projectModalVid.style.display = "block";   // show video
      }
      // ✅ Otherwise fallback to image
      else {
        const imgEl = this.querySelector("[data-projects-avatar]");
        if (imgEl?.src) {
          projectModalImg.src = imgEl.src;
          projectModalImg.alt = imgEl.alt || "Project image";
          projectModalImg.style.display = "block"; // show image
        }
      }
